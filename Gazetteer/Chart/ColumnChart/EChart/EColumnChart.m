...
#import "EFloatBox.h"
#define BOTTOM_LINE_HEIGHT .5
#define HORIZONTAL_LINE_HEIGHT 0.5
#define Y_COORDINATE_LABEL_WIDTH 60
...
- (void)setDataSource:(id<EColumnChartDataSource>)dataSource
{
    if (_dataSource != dataSource)
    {
        ...
        
        {
            ...
            totalColumnsRequired = (int)[_dataSource numberOfColumnsPresentedEveryTime:self];
            ...
            if (_showHorizontalLabelsWithInteger)
            {
                ...
                for (int i = 0; i <= horizontalLabelsCount; i ++)
                {
                    ...
                    eColumnChartLabel.text = [[NSString stringWithFormat:@"%i ", (int)(valueGap * (horizontalLabelsCount - i))] stringByAppendingString:[_dataSource highestValueEColumnChart:self].unit];;
            ...
            else
            {
                ...
                _fullValueOfTheGraph = (_rowCount -1) * _valueGap;
                ...
                for (int i = 0; i < _rowCount; i++)
                {
                    ...
                    EColumnChartLabel *eColumnChartLabel = [[EColumnChartLabel alloc] initWithFrame:CGRectMake(-1 * Y_COORDINATE_LABEL_WIDTH, -heightGap / 2.0 + heightGap * i, Y_COORDINATE_LABEL_WIDTH, heightGap)];
                    [eColumnChartLabel setTextAlignment:NSTextAlignmentCenter];
                    NSNumber *val =[[NSNumber alloc] initWithInt:_valueGap * ((_rowCount -1) - i) ] ;
                    if ([val intValue] == 0) {
                        eColumnChartLabel.text = [NSString stringWithFormat:@"       %@", [formatter stringFromNumber:val] ];
                    } else {
                        eColumnChartLabel.text = [formatter stringFromNumber:val];
                    }
                    
                    if ([_dataSource respondsToSelector:@selector(colorForLabel)]) {
                        eColumnChartLabel.textColor = [_dataSource colorForLabel];
                    }
                    if ([_dataSource respondsToSelector:@selector(fontForLabel)]) {
                        eColumnChartLabel.font = [_dataSource fontForLabel];
                    }
...
- (void)moveLeft
{
    ...
    int index = (int)(_leftMostIndex + 1 * DIRECTION);
    ...
    int totalColumnsRequired = (int) [_dataSource numberOfColumnsInEColumnChart:self];
    for (int i = 0; i < totalColumnsRequired; i++)
    {
...
- (void)moveRight
{
    ...
    int index = (int)(_rightMostIndex - 1 * DIRECTION);
    EColumnDataModel *eColumnDataModel = [_dataSource eColumnChart:self valueForIndex:index];
    if (nil == eColumnDataModel) return;
